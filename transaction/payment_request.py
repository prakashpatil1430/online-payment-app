...
def AmountRequestProcess(request, account_number):
    ...
    if request.method == "POST":
        ...
        return redirect("transaction:amount-request-confirmation",
...
def AmountRequestConfirmation(request, account_number, transaction_id):
    ...
    return render(request,
                  "payment_request/amount-request-confirmation.html", context)
...
def AmountRequestFinalProcess(request, account_number, transaction_id):
    ...
    if request.method == "POST":
        ...
        if pin_number == request.user.account.pin_number:
            ...
            messages.success(
                request, "Your payment request has been sent successfully.")
            return redirect("transaction:amount-request-completed",
                            account.account_number, transaction.transaction_id)
        else:
            messages.warning(
                request, "Incorrect PIN number. Please try again.")
            return redirect("account:dashboard")
    else:
...
def RequestCompleted(request, account_number, transaction_id):
    ...
    return render(request,
                  "payment_request/amount-request-completed.html", context)
...
def settlement_confirmation(request, account_number, transaction_id):
    ...
    return render(request,
                  "payment_request/settlement-confirmation.html", context)
...
def settlement_processing(request, account_number, transaction_id):
    ...
    if request.method == "POST":
        pin_number = request.POST.get("pin-number")
        if pin_number == request.user.account.pin_number:
            if sender_account.account_balance <= 0 or \
                ...
                messages.success(
                    request, f"Settled to \
                    {account.user.kyc.full_name} was successfull.")
                return redirect("transaction:settlement-completed",
                                account.account_number,
                                transaction.transaction_id)

        else:
            messages.warning(request, "Incorrect Pin")
            return redirect("transaction:settlement-confirmation",
                            account.account_number, transaction.transaction_id)
    else:
...
def SettlementCompleted(request, account_number, transaction_id):
    ...
    return render(request, "payment_request/settlement-completed.html",
                  context)
